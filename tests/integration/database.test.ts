...
// These assertions count rows, so they run in a client of their own: against a
// shared workspace another suite's fixtures would drift the totals.
const ws = dbUp ? await makeWorkspace("db") : { id: "", scope: {} };
...
d("database persistence", () => {
  ...
  beforeAll(async () => {
    const cv = await prisma.cVFile.create({
      data: { workspaceId: ws.id, sourceType: "MANUAL", fileName: `${TAG}_Rahul.pdf`, mimeType: "application/pdf", fileHash: sha256Hex(TAG), fileSize: 123, storagePath: `test/${TAG}.pdf`, status: "PROCESSED" },
    });
    cvFileId = cv.id;
  ...
  it("persists and reads back a candidate with its file and jobs", async () => {
    const detail = await getCandidateDetail(ws.scope, cvFileId);
  ...
  it("search + filters are database-backed and paginated", async () => {
    ...
    const byName = await listCandidates(ws.scope, candidateFiltersSchema.parse({ q: "rahul sh" }));
    expect(byName.items.some((i) => i.id === cvFileId)).toBe(true);
    const byFile = await listCandidates(ws.scope, candidateFiltersSchema.parse({ q: TAG }));
    expect(byFile.total).toBe(1);
    const byStatus = await listCandidates(ws.scope, candidateFiltersSchema.parse({ status: "NEEDS_REVIEW", role: "Java", source: "MANUAL" }));
    expect(byStatus.items.some((i) => i.id === cvFileId)).toBe(true);
    const none = await listCandidates(ws.scope, candidateFiltersSchema.parse({ q: TAG, status: "FAILED" }));
    expect(none.total).toBe(0);
    const paged = await listCandidates(ws.scope, candidateFiltersSchema.parse({ q: TAG, page: 2, pageSize: 1 }));
  ...
  it("manual correction sets is_manually_corrected and clears the review flag", async () => {
    const c = await updateCandidate(ws.scope, cvFileId, { jobRoleAppliedFor: "Senior Java Developer" });
  ...
  it("rejects invalid manual corrections", async () => {
    await expect(updateCandidate(ws.scope, cvFileId, { phoneNumber: "12" })).rejects.toThrow(/Invalid phone/);
    await expect(updateCandidate(ws.scope, cvFileId, { candidateName: "Infosys Ltd" })).rejects.toThrow();
    await expect(updateCandidate(ws.scope, "does-not-exist", { candidateName: "X Y" })).rejects.toThrow(/not found/);
  });

  it("normalises a manually entered phone number", async () => {
    const c = await updateCandidate(ws.scope, cvFileId, { phoneNumber: "098765 43211" });
  ...
  it("enforces unique Drive file ids (duplicate prevention)", async () => {
    const data = { workspaceId: ws.id, sourceType: "GOOGLE_DRIVE" as const, sourceFileId: `${TAG}_drive1`, fileName: `${TAG}_drive.pdf`, mimeType: "application/pdf", fileHash: `drive:${TAG}`, fileSize: 1 };
  ...
  it("dashboard stats aggregate by status", async () => {
    const s = await getDashboardStats(ws.scope);
